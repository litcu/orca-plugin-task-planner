...
| [10-项目数据流.md](./10-项目数据流.md) | 项目标签、结构归属、手动加入项目、项目进度与项目 tab。 |
...
| --- | --- | --- | --- |
| 批量查询 | `orca.invokeBackend("get-blocks-with-tags", [tagAlias])` | 全部任务、项目、激活任务、计时器全局扫描、删除依赖清理 | 高：按标签查询多条任务/项目数据，数据多时是主要耗时来源。 |
...
- 查任务列表、项目、激活任务、仪表盘、计时器候选时，优先看 [04-查询与视图数据流.md](./04-查询与视图数据流.md)、[05-依赖与评分数据流.md](./05-依赖与评分数据流.md)、[07-任务计时器数据流.md](./07-任务计时器数据流.md)、[10-项目数据流.md](./10-项目数据流.md)。
