...
flowchart LR
  ...
  Refs --> ProjectTagRef["项目标签引用 type=2 alias=schema.projectTagAlias"]
  TaskTagRef --> RefData["ref.data: 用户可见任务字段"]
  ProjectTagRef --> ProjectData["project ref.data: 项目状态、日期、标签、备注"]
...
| `projects` | `项目` | block refs, scope 指向项目标签 | 任务标签 `ref.data` |
...
默认 alias：`Project`。用户可以通过设置项 `projectTagName` 修改。

项目不是任务：同一 block 即使同时带有 `Task` 和 `Project` 标签，也会优先按项目处理，不进入普通任务列表、激活任务、My Day 或计时器候选。

项目标签同时承担项目身份识别和项目级属性 schema。`ensureProjectTagSchema()` 会在启动和项目标签名变更后补齐下列属性；项目进度与结构汇总仍由 `project-engine.ts` 动态读取项目 block 树和任务 `Projects` 字段后生成，不落库。

中文字段：

| 逻辑字段 | 中文属性名 | 类型 | 存储位置 |
| --- | --- | --- | --- |
| `status` | `项目状态` | text choices, single | 项目标签 `ref.data` |
| `startTime` | `项目开始时间` | datetime | 项目标签 `ref.data` |
| `dueTime` | `项目截止时间` | datetime | 项目标签 `ref.data` |
| `labels` | `项目标签` | text choices, multi | 项目标签 `ref.data` |
| `note` | `项目备注` | text | 项目标签 `ref.data` |
...
| `projects` | 任务标签 `ref.data`，Block refs，scope 为项目标签 alias |
...
| `projectTagName` | 项目标签 alias，影响项目读取、项目选择和项目标签重命名。 |
