...
| 文档 | 覆盖范围 |
| --- | --- |
| [01-运行时启动与注册.md](./数据流/01-运行时启动与注册.md) | 插件启动、设置同步、任务标签 schema 初始化、运行时模块注册与卸载。 |
| [02-数据模型与存储.md](./数据流/02-数据模型与存储.md) | 任务标签 schema、`ref.data`、`_mlo_task_meta`、`_mlo_task_timer`、插件私有数据。 |
| [03-任务生命周期数据流.md](./数据流/03-任务生命周期数据流.md) | 任务创建、属性编辑、状态循环、删除、移动、子任务和父任务链接。 |
| [04-查询与视图数据流.md](./数据流/04-查询与视图数据流.md) | 全部任务读取、任务树、任务管理面板、仪表盘、收藏、即将到期、回顾和筛选视图。 |
| [05-依赖与评分数据流.md](./数据流/05-依赖与评分数据流.md) | 激活任务计算、依赖判定、顺序子任务、循环依赖、评分与排序。 |
| [06-我的一天数据流.md](./数据流/06-我的一天数据流.md) | My Day 私有状态、跨日重置、今日日志镜像/引用块同步、日程排期。 |
| [07-任务计时器数据流.md](./数据流/07-任务计时器数据流.md) | 任务计时数据、直接计时、番茄钟、全局计时器、行内控件、状态联动。 |
| [08-回顾与重复任务数据流.md](./数据流/08-回顾与重复任务数据流.md) | 回顾规则、标记已回顾、重复规则、完成后推进下一周期。 |
| [09-自定义视图与筛选数据流.md](./数据流/09-自定义视图与筛选数据流.md) | 自定义视图持久化、筛选树、字段发现和匹配流程。 |
...
| 文档 | 覆盖范围 |
| --- | --- |
| [My Day 引用块实现.md](./实现说明/My Day 引用块实现.md) | My Day 从镜像块迁移到引用块的最终实现、引用显示名清洗、旧数据归一化。 |
